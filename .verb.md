...
| --- | --- | --- | --- |
| `basename`            | `boolean`      | `false`     | If set, the pattern will be matched against the basename of the path, regardless of whether or not the pattern contains a slash.  For example, `a?b` would match the path `/xyz/123/acb`, but not `/xyz/acb/123`. |
